...
- **Échelle typo agrandie « +1 cran »** (26 juil. 2026) : override `theme.extend.fontSize` dans `tailwind.config.js` → tous les `text-*` de l'app un peu plus grands, réglable d'un seul endroit. Changement de config Tailwind → **relancer Metro avec `-c`** pour le voir. ⚠️ **Exception : les écrans d'inscription `app/(auth)/*`** gardent les tailles d'origine via des **valeurs arbitraires en px** (`text-[24px]`, `text-[16px]`, …) et **pas** les classes `text-*` (qui seraient scalées). Choix des arbitraires plutôt que de clés de config : elles sont régénérées au simple fast-refresh du fichier (pas besoin de `-c`). Ne pas remettre de `text-xs/sm/base/...` dans `(auth)`.
