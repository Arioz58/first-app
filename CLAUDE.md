...
```
app/
├── _layout.tsx          # Layout racine — vérif JWT expiry + handler SESSION_EXPIRED global (⚠️ **sans effet si l'on est déjà dans `(auth)`** : une requête tardive d'un écran quitté renvoyait sinon à l'accueil quelqu'un en train de s'inscrire, le ramenant à la première étape à chaque tentative) + socket + push + `initTheme()` (thème restauré avant rendu) + `KeyboardProvider` + listeners sociaux (notifs in-app) + **ouverture depuis une notification** (deep link → `/chat/<conversationId>`)
├── globals.css
...
├── MessageText.tsx      # Texte d'une bulle : formatage `*gras*`/`_italique_`/`~barré~`/`` `mono` ``, liens **tous** cliquables, repli « Voir plus » au-delà de 8 lignes. ⚠️ Au-delà de 400 caractères (ou 8 retours à la ligne) la bulle est **tronquée dès le premier rendu, sans mesure** : rendre un message de 8 500 caractères en entier pour le mesurer donnait une cellule de 4 300 px le temps d'une image, et la position du fil partait avec (bug du 11/09). En deçà, le débordement reste **mesuré** — c'est là que la mesure est utile et sans danger
...
lib/
├── api.ts               # Fetch wrapper — JWT Bearer + auto-refresh + handler SESSION_EXPIRED global. ⚠️ Le renouvellement distingue `refused` (le serveur REFUSE le jeton → session effacée) de `unreachable` (requête non aboutie : réseau, **5xx**, serveur qui redémarre → `NETWORK_UNAVAILABLE`, **session intacte**). Les confondre déconnectait pour de bon un utilisateur valide dès qu'un déploiement tombait pendant un renouvellement
...
├── toasts.ts            # Store externe des bandeaux d'alerte — **un bandeau par message** (regrouper par conversation faisait que la pile n'apparaissait jamais), 5 gardés, 3 visibles pile fermée, 5 s ; ouvrir la pile SUSPEND l'expiration
...
└── modules/
    ...
    │                               # ⚠️ `deliver.service.ts` = LE seul envoi de message (création, diffusion,
...
PATCH /users/me/privacy                            → met à jour la matrice de confidentialité (valeurs validées serveur : triple everyone/friends/nobody, friend_requests everyone/friends_of_friends/nobody, locationEnabled bool, **readReceipts bool**). ⚠️ Les 5 champs de **visibilité** acceptent en plus **`friends_except`** (« mes amis, sauf… ») ; les 3 champs de **contact** (messages/calls/friend_requests) la refusent — y écarter quelqu'un serait du blocage, qui existe déjà. La liste d'exclus voyage dans le MÊME appel (`<champ>Except: string[]`, ex. `privacyPhotoExcept`) et le serveur écrit valeur + liste dans une **transaction** : un « sauf » enregistré sans sa liste se comporterait comme « mes amis » et montrerait à quelqu'un qu'on venait d'écarter. Remplacement INTÉGRAL de la liste (une liste vide la vide), ids **filtrés sur les amis réels** (ils viennent du client)
GET  /users/me/privacy/exceptions                  → les exclusions en vigueur, par réglage → `{ privacyPhoto: [{id,name,photoUrl}], privacyBio: [...], ... }` (toujours les 5 clés, même vides). ⚠️ Route séparée et non un champ de `GET /users/me` : seul l'écran de confidentialité en a besoin
...
POST /conversations/:id/messages                  → envoyer un message SANS socket. ⚠️ Existe pour la **réponse depuis une notification** : l'app ferme son socket en arrière-plan (c'est ce qui permet aux notifications d'exister), donc au moment où l'on répond à l'une d'elles il n'y a plus de socket. Passe par `deliverMessage`, la MÊME fonction que `send_message` — mêmes effets (blocage, `whoCanSend`, éphémères, albums, `alert`, push, accusés). Refus tous en **403** : distinguer « pas membre » de « bloqué » révélerait l'existence d'un blocage
...
GET  /geocode/reverse?lat=&lon=&lang=      → adresse lisible d'un point (proxy Nominatim/OpenStreetMap, sans clé). ⚠️ Existe pour la même raison que le proxy Giphy et `unfurl` : le mobile géocode **sur l'appareil** (services de l'OS, rien ne sort du téléphone), mais le navigateur n'a pas d'équivalent — le faire depuis la page livrerait la position exacte de l'utilisateur et son IP à un tiers. Cache Redis 30 j sur des coordonnées **arrondies** à ~11 m (sans quoi le GPS ne rendant jamais deux fois la même valeur, le cache ne servirait jamais) ; les échecs sont mémorisés aussi. Adresse composée depuis les champs structurés, jamais `display_name` (illisible : il empile tout jusqu'au pays). Authentifié + 60/h/utilisateur : ouvert, ce serait un relais de géocodage portant **notre** identification auprès du service
...
// Serveur → Client
new_message(message)                              → refus si blocage (conv directe) ; pièces jointes + `hasLink` (détection d'URL) + `expiresAt` si la conv est en éphémère ; + push aux destinataires **acceptés** dont **aucune app mobile n'est ouverte** (`isUserOnMobile`) — ⚠️ règle changée le 11/09 : le critère était « a déjà la conversation ouverte » (room `conv:`), mais le `setNotificationHandler` de l'app affiche les notifications **même au premier plan**, si bien qu'un message reçu pendant qu'on est sur un autre écran donnait une bannière système **en plus** du bandeau in-app. App ouverte = bandeau, app fermée ou en arrière-plan = notification. ⚠️ Juste **uniquement** parce que le socket mobile se ferme en arrière-plan (`pauseSocket`) (⚠️ les sockets **web** sont exclus de ce calcul depuis le 10/09 : un onglet oublié rendait le téléphone muet — voir la note « Plateforme du socket » plus bas) (titre = nom de l'expéditeur en direct, **nom du groupe** en groupe avec « Alice : … » dans le corps ; illustration = photo du groupe ou de l'expéditeur ; `data` porte `conversationId`, `senderName`, `avatarUrl` — lus par l'extension iOS) (pas de push aux membres en « demande » accepted=false → badge uniquement, ni aux membres ayant coupé les notifs `mutedUntil` dans le futur). ⚠️ **Messages système** (`type:'system'`, `content` = JSON `{ k: clé i18n, by, ...params }`) émis via `createSystemMessage` (`messages.service`) sur événements groupe/éphémère (member_added/removed/left, group_created/renamed, ephemeral_on/off) : rendus en **bandeau centré** côté app (traduits via `system.*` selon la langue du lecteur), **exclus des non-lus** (`type != 'system'` dans le COUNT), exclus de la recherche, pas de push. Pas de message « chiffrement bout en bout » tant que l'E2E n'existe pas (V2).
...
conversation_updated({ conversationId, message, alert? }) → **room `user:<id>`** (≠ `new_message` qui part dans `conv:<id>`) : met à jour la LISTE des conversations même si la conv n'a jamais été ouverte dans la session. Payload allégé volontairement (l'objet `message` complet embarque `sender` avec téléphone + token FCM). Émis à tous les membres, **émetteur inclus** (ses autres appareils). ⚠️ **`alert`** (`{ title, body, photoUrl, isGroup }`, composé par `messageSummary` — le même helper que la notification push) n'est joint qu'aux membres qu'il y a lieu de **prévenir** : pas l'émetteur, pas les conversations en **sourdine**, pas les **demandes** non acceptées, pas les médias **suivants** d'un album. Le client ne peut pas trancher (il ne connaît pas forcément la conversation, et la sourdine est une donnée par membre) — il se contente d'afficher le bandeau (`ToastStack`). N'y remettre **aucun** autre champ de `sender` : téléphone et jeton push
...
> **Battements de cœur** (13 sept. 2026) : `pingInterval` / `pingTimeout` à **10 s** côté serveur (`new Server(...)` dans `src/lib/socket.ts`), au lieu des défauts de socket.io (25 s + 20 s). ⚠️ Deux sortes de coupures, une seule pose problème : un serveur qu'on **arrête** ferme proprement ses connexions et le client l'apprend aussitôt ; un réseau qui **disparaît** (tunnel, Wi-Fi coupé, mode avion) n'envoie rien du tout, et seul l'arrêt des battements le démasque — jusqu'à **45 s** avec les défauts, d'où un bandeau « hors ligne » qui tardait. ⚠️ Ne pas descendre beaucoup plus bas : `pingTimeout` doit absorber une latence dégradée (réseau mobile lent, changement d'antenne), sinon on déconnecte des clients parfaitement joignables. Le coût ne pèse que pendant l'usage actif, l'app fermant son socket en arrière-plan.

> **Plateforme du socket** (10 sept. 2026) : chaque client déclare `platform` au handshake (`auth: { token, platform }` — `mobile` côté app, `web` côté Next.js), lue dans `src/lib/socket.ts` et exposée par **`isUserOnMobile(userId)`**. ⚠️ Elle sert **uniquement** à décider d'un push, jamais à autoriser quoi que ce soit : elle vient du client, donc elle se falsifie — au pire on se prive de ses propres notifications. ⚠️ **Défaut `mobile`** quand rien n'est déclaré : les builds **déjà installés** n'envoient pas ce champ, et les traiter comme du web les rendrait muets — le défaut doit toujours être l'ancien comportement. ⚠️ **Ne pas** substituer `isUserOnMobile` à `isUserOnline` ailleurs : la **présence** (« en ligne / vu le… ») et les **accusés de réception** restent basés sur `isUserOnline` — quelqu'un connecté depuis son navigateur est bel et bien en ligne, et un message parvenu à un onglet est bel et bien parvenu. ⚠️ Conséquence assumée : conversation ouverte sur l'ordinateur, le téléphone s'allume **aussi** — c'est ce que fait WhatsApp, et le serveur ne peut pas savoir quel écran est regardé. ⚠️ Le socket mobile ferme en arrière-plan (`pauseSocket`), donc un socket mobile ouvert signifie « app affichée à l'écran » : c'est ce qui rend ce calcul juste.

> **Notifications (social)** : `social/notify.service.ts` — push **uniquement si le destinataire n'a pas son app mobile ouverte** (`isUserOnMobile`, ⚠️ **pas** `isUserOnline` — un onglet web ouvert supprimait la notification du téléphone), sinon event socket → notif **in-app locale** (`expo-notifications`) côté `_layout` (pas de doublon push/in-app). Corps localisé selon la langue du destinataire (serveur) ou via i18n (client).
...
- Les GIFs passent par le **proxy serveur** `GET /giphy` (la clé n'est plus dans le bundle, cf. Sécurité) ; si le backend n'a pas de clé, il répond **503** et `GiphyPicker` affiche l'écran « indisponible » au lieu de planter.
...
- **Vocaux** : le lecteur natif vit dans **`lib/voicePlayback.ts`** (`createAudioPlayer`, hors de l'arbre React), pas dans la bulle. ⚠️ `useAudioPlayer` s'appuie sur `useReleasingSharedObject`, qui **libère le lecteur au démontage** — le son se coupait donc en quittant la conversation, ce qui rendait le mini-player impossible. Conséquence : les bulles vocales n'ont **aucun** état natif (rien n'est alloué au défilement) et le `VoiceMiniPlayer` peut prendre le relais hors du chat.
...
- **État de la connexion** (13 sept. 2026) : bandeau en tête de liste, **rouge** hors ligne (avec « Réessayer ») puis **vert** « Connexion rétablie » 3 s à la reconnexion. Le vert n'apparaît **qu'après une vraie coupure**, sinon il clignoterait à chaque chargement réussi. Porté à l'identique sur le **web** (`ConversationList.tsx`), même durée des deux côtés.
  - ⚠️ **AUCUN réessai périodique**, nulle part. La coupure est lue sur **l'état du socket** (`disconnect`, et `connect_error` au retour d'arrière-plan) et non en envoyant des requêtes : sans réessai, un « échec » n'existe que s'il y a une requête — couper le réseau ne produisait donc rien tant qu'aucune ne partait. Une relance périodique, elle, s'acharnerait quand il ne faut pas : si c'est le **serveur** qui faiblit, chaque client le rappellerait toutes les quelques secondes, et `GET /conversations` fait un COUNT par conversation.
  - ⚠️ `io client disconnect` est **ignoré** : c'est notre propre `pauseSocket()` en arrière-plan. Sans ce test, revenir sur l'app afficherait un bandeau alors que tout va bien.
  - **Rattrapage à la reconnexion** : une seule requête, déclenchée par l'événement `connect`. Ce n'est **pas** du polling — le socket retente de lui-même (délai croissant plafonné à 5 s), on écoute son succès. ⚠️ **Pas de garde** « ne pas recharger si on vient de le faire » : un délai arbitraire ferait sauter un rattrapage légitime et laisserait un trou dans la liste, cette fois sans bandeau pour le dire. Si des rafales apparaissaient, la réponse serait un **delta serveur**, pas une temporisation.
  - ⚠️ Tout minuteur de cet écran est **annulé au démontage**. Un réessai qui survivait à son écran rappelait une fonction d'un composant disparu, laquelle en reprogrammait un autre : l'onboarding revenait à sa première étape en boucle, et `clearTokens()` répété pouvait effacer les jetons tout juste obtenus à l'inscription.
...
- **Socket coupé en arrière-plan** (`AppState` → `pauseSocket()` / `resumeSocket()` dans `app/_layout.tsx`) : le serveur ne pousse qu'à qui n'a pas d'app **mobile** ouverte, et une app en arrière-plan gardait son socket ouvert — donc restait « en ligne » quelques secondes, pendant lesquelles les messages partaient en événement socket dans le vide, **sans notification**. On ferme donc explicitement sur `background` (pas sur `inactive` : bascule d'app, centre de contrôle, appel entrant). Effet voulu : la présence « en ligne / vu le … » devient exacte.
  - ⚠️ **Handshake refusé = mort silencieuse du temps réel** (mesuré le 11/09) : socket.io **ne retente pas** après un refus du middleware d'authentification — une tentative, puis plus rien. Le jeton d'accès ne vivant que 15 min, toute reconnexion (réseau, veille, redémarrage du serveur) repartait avec un jeton périmé et l'app cessait de recevoir quoi que ce soit, **sans aucun signe**. Côté web le symptôme était « il faut recharger la page pour voir les nouveaux messages ». Les deux clients écoutent donc `connect_error`, renouvellent le jeton (`refreshAccessToken`, exporté par `api.ts`) et **rappellent `connect()`**. ⚠️ On ne renouvelle **que** si le jeton est expiré : sinon le serveur est injoignable, il n'y a rien à renouveler — et c'est ce qui empêche la boucle.
  - ⚠️ `refreshAccessToken` **partage sa promesse en vol** des deux côtés : plusieurs requêtes parallèles reçoivent le même 401, et le serveur invalidant l'ancien jeton de rafraîchissement à chaque usage, deux renouvellements simultanés en feraient échouer un — donc une déconnexion à tort.
...
- **Répondre depuis la notification** (22 sept. 2026, ⚠️ pas de rebuild requis) : catégorie `message` + action `reply` à champ de saisie (`registerReplyCategory`, `lib/notifications.ts`), `categoryId` envoyé par le serveur sur les seules notifications de MESSAGE, et `sendReplyFromNotification` qui envoie par **REST** puis marque la conversation lue. ⚠️ `opensAppToForeground: false` est ce qui répond à la demande (ne pas ouvrir l'app), mais `expo-notifications` documente qu'alors l'écouteur **ne se déclenche pas si l'app a été TUÉE** — le texte est perdu. Mesure sur appareil réel à faire avant de décider du natif. ⚠️ **Garde anti-doublon persistée** (SecureStore) : une réponse déjà traitée reste la « dernière réponse » rendue au lancement suivant par `getLastNotificationResponseAsync`, et sans garde chaque démarrage à froid renverrait le même message.
...
- **Profile** : matrice de confidentialité — `privacyPhoto`, `privacyBio`, `privacyLastSeen`, `privacyLocation` (défaut `friends`), `privacyPhone` = **champs de VISIBILITÉ** (`everyone` | `friends` | **`friends_except`** | `nobody`) ; `privacyMessages`, `privacyCalls`, `privacyFriendRequests` = **champs de CONTACT** (pas de `friends_except`) + `locationEnabled` (bool, défaut false) + **`readReceipts`** (bool, défaut true — accusés de lecture)
- **PrivacyException** `{ userId, field, excludedId }` (migration `privacy_exceptions`, unique `[userId, field, excludedId]`, index `[excludedId, userId]`, cascade des deux côtés) — « mes amis, SAUF cette personne », **pour un réglage donné**. ⚠️ Une ligne par (propriétaire, réglage, exclu) et non une liste en colonne Json : la question posée en lecture est « de quels réglages ce visiteur est-il exclu ? », et elle se répond par UNE requête indexée. ⚠️ L'exclusion est **par réglage** : quelqu'un peut perdre la photo et garder la bio. ⚠️ `canSee(rule, viewer, target, field)` — le `field` est **obligatoire** : sans lui, `friends_except` ne se tranche qu'en devinant, et les deux réponses sont mauvaises (montrer trahit l'exclusion auprès de l'exclu lui-même, masquer revient à appliquer « personne »). `excludedFieldsFor(owner, viewer)` charge les cinq d'un coup pour l'affichage d'un profil, ce qui garde le filtre **synchrone**
- **User** : `lastSeenAt`
...
- **Phase 6 ✅** — `social/notify.service.ts` : demande d'ami **reçue** + **acceptée** → push si aucune app mobile ouverte (`isUserOnMobile`), sinon event socket → **notif in-app locale** (listeners globaux dans `_layout`). Demande de message = badge seul (Phase 5). ⏳ Appel entrant = Mois 4 (Agora).
...
- **Deux fuites de photo de profil restées ouvertes** (constatées le 21 sept. 2026 en livrant « mes amis, sauf… »). ⚠️ Ce ne sont PAS des régressions des exceptions : le trou est d'origine, et même « Personne » fuyait — la nouveauté n'a fait que le rendre visible. Les quatre chemins d'affichage ont été colmatés (`makePhotoGate` sur `getUserConversations`, `getConversationDetails`, `getFriends`) ; restent :
  1. **La photo de l'expéditeur voyage dans `MESSAGE_SELECT`**, donc dans l'historique ET dans l'event socket `new_message`. ⚠️ **Aucun client ne l'affiche** (vérifié dans les deux dépôts : les bulles n'utilisent que `sender.name`) — la fuite est dans la charge utile, pas à l'écran. Pas retirée à la volée pour deux raisons : le serveur s'en sert pour l'image des notifications push, et `new_message` part dans une room avec **une seule** charge pour tout le monde — masquer par destinataire imposerait d'émettre socket par socket.
  2. **L'image de la notification push ignore `privacyPhoto`** : elle est composée par `sendPushToMany` sans passer par la matrice.
  À trancher : soit retirer le champ du `select` et recomposer l'image du push à part, soit l'assumer explicitement.
- ✅ **`GIPHY_API_KEY` sortie du bundle** (5 sept. 2026) puis **clé régénérée et ancienne révoquée** (13 sept. 2026) : proxy serveur `GET /giphy` (`src/modules/giphy/`), clé en variable d'environnement backend (`first-app-backend/.env` **uniquement**, et Railway), mobile et web y passent tous les deux. ⚠️ Ne **jamais** la remettre côté client : `first-app` et `first-app-web` sont des dépôts **PUBLICS**, et c'est ainsi que la précédente s'est retrouvée lisible de tous. ⚠️ Réécrire l'historique git ne « dé-publie » rien (clones et caches GitHub) : seule la **révocation** compte. ⚠️ Pour vérifier une rotation de clé : `nodemon` ne surveille pas le `.env` (le serveur garde l'ancienne en mémoire — le relancer), et le cache Redis `giphy:trending` (TTL 10 min) masque le problème en servant encore des tendances — tester par une **recherche** (`?q=`), qui ne passe pas par le cache. Le `cid` des URL Giphy, en base64, porte l'empreinte de la clé réellement utilisée.
...
- **`lib/config.ts`** contient aussi `PRIVACY_URL` / `PRIVACY_POLICY_VERSION` (⚠️ URL placeholder) et `INVITE_URL`. ⚠️ **Plus aucune clé d'API** : Giphy est passée par le proxy serveur, ne rien réintroduire ici.
