...
- ✅ **`GIPHY_API_KEY` sortie du bundle** (5 sept. 2026) : proxy serveur `GET /giphy` (`src/modules/giphy/`), clé en variable d'environnement backend, mobile et web y passent tous les deux. ⚠️ **Reste à faire** : la clé étant **déjà commitée**, elle doit être **révoquée et régénérée** sur developers.giphy.com. Le proxy rend cette rotation possible sans publier d'app — mais les builds mobiles **déjà installés** appellent encore Giphy en direct, leurs GIFs cesseront de fonctionner jusqu'au prochain build EAS.
