...
- **Liste INVERSÉE** (7 août 2026) : le fil se rend depuis le bas (`inverted`), donc charger de l'historique est un ajout **à la fin**, qui ne touche jamais ce qui est au-dessus du regard. C'est ce qui a supprimé le sursaut à la remontée — en liste normale, les messages insérés en tête sortent de la zone visible, y sont virtualisés avec une hauteur **estimée**, et chaque cellule montée remplace cette estimation, décalant le point d'ancrage image après image. ⚠️ Conséquences à connaître : `data` = `displayRows` (copie retournée, `rows` restant chronologique pour le regroupement) ; le message précédent est à `index + 1` ; **les extrémités sont échangées** (l'en-tête de liste s'affiche en bas, le pied en haut) ; le bas du fil est à l'**offset zéro** (`scrollToOffset({offset: 0})`, jamais `scrollToEnd`) ; `viewPosition: 1` vise le haut de l'écran. `maintainVisibleContentPosition` a été supprimé, il n'a plus d'objet.
