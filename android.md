...
- [ ] **Tâche de fond d'accusé de réception** (`lib/deliveryReceipt.ts`, 6 août 2026) : sur iOS c'est l'extension de notification qui prévient le serveur qu'un message est arrivé ; Android n'a pas d'équivalent, d'où une tâche `expo-notifications` + TaskManager enregistrée au démarrage. **Jamais exécutée pour l'instant.** À vérifier : que la tâche se déclenche app en arrière-plan **et** app tuée ; que la charge utile arrive bien à l'endroit attendu (`extract()` sonde plusieurs emplacements, faute de garantie sur la forme) ; et que le serveur est joignable depuis l'appareil (`PUBLIC_URL` = IP Wi-Fi en dev, pas `localhost`).
...
- [ ] **Réponse directe depuis la notification** (22 sept. 2026) : l'action à champ de saisie est déclarée une seule fois pour les deux plateformes (`registerReplyCategory`, `lib/notifications.ts`), et `expo-notifications` la traduit en **`RemoteInput`** sur Android. Rien n'a été vérifié là-bas. À contrôler : que le champ apparaît bien sur la notification développée ; que le texte revient dans `response.userText` ; que l'application **ne passe pas au premier plan** (`opensAppToForeground: false`) ; et surtout **app tuée**, où Android — contrairement à iOS — relance souvent le processus pour délivrer l'action, ce qui pourrait couvrir gratuitement le cas qu'iOS perd. ⚠️ La notification doit porter le canal `messages` **et** la catégorie `message` ; sur Android la catégorie ne sert qu'à porter les actions, elle n'a pas le rôle système qu'elle a sur iOS.
...
- [?] **`ProgressiveBlur`** (haut du fil de discussion, bas de l'aperçu de position) : plusieurs couches de flou masquées. Coût à surveiller sur un appareil d'entrée de gamme — prévoir de le désactiver sur Android si ça saccade au défilement ou à l'ouverture du clavier. Le profil du dégradé est calculé de la même façon sur les deux plateformes, mais Android n'empile que **3** couches contre 4 sur iOS (`ANDROID_LAYERS`) : la montée reste quasi linéaire, seuls les paliers sont un peu plus larges. À juger visuellement — si des marches se voient, monter à 4 avant de toucher à la courbe.
...
- [ ] **Coins continus (`borderCurve`)** : `lib/radius.ts` pose `borderCurve: 'continuous'` sur les surfaces (bulles, en-tête de chat, drawers, tuiles, aperçus). C'est la superellipse d'iOS, celle qui évite le pincement aux tangentes d'un arc de cercle. **Android l'ignore silencieusement** et rend un arc ordinaire : les rayons sont identiques sur les deux plateformes, seule la finesse de la courbe manque. Aucun repli raisonnable — il faudrait redessiner chaque surface en SVG (`react-native-svg`, tracé de superellipse) et renoncer aux fonds NativeWind, pour un gain que personne ne verra sans comparer côte à côte. À trancher : accepter l'écart (recommandé) ou chiffrer le repli SVG sur les seules grandes surfaces (drawers, en-tête). Vérifier au passage qu'aucun coin ne paraît « dur » à côté des composants Material.
...
- [?] **Recul de l'écran sous les drawers** (`lib/sheetRecede.ts`) : à l'ouverture d'une feuille, l'écran du dessous rétrécit et ses coins s'arrondissent, sur fond noir. Le code est commun aux deux plateformes (transformation Reanimated), mais le `Modal` de React Native est une **fenêtre Dialog distincte** sur Android : à confirmer que l'activité derrière reste bien visible et transformée, et que le noir découvert autour ne laisse pas passer la barre de navigation gestuelle. Vérifier aussi que la barre d'onglets recule avec le reste.
